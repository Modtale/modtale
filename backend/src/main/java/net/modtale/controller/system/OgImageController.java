...
public class OgImageController {
    ...
    private static final Color BANNER_BG = new Color(30, 41, 59);
    private static final Color TEXT_PRIMARY = Color.WHITE;
    private static final Color TEXT_SECONDARY = new Color(148, 163, 184);
    private static final Color TEXT_DESC = new Color(148, 163, 184);
    ...
    public ResponseEntity<ByteArrayResource> generateOgImage(
        ...
        try {
            ...
            String cacheKey = project.getId() + ":" + formatName;
    ...
    private String generateEtag(Project project) {
        try {
            String raw = project.getId() + "|" + project.getUpdatedAt() + "|" + project.getTitle() + "|" + project.getDownloadCount() + "|" + project.getFavoriteCount();
    ...
    private byte[] renderImage(Project project, BufferedImage banner, BufferedImage icon, String format, boolean isJpg) throws IOException {
        ...
        drawCardBackgroundWithBanner(g2d, project, banner, cardShape, cardX, cardY, cardW);
        drawProjectContent(image, g2d, project, icon, cardX, cardY, cardW, cardH);
    ...
    private void drawCardBackgroundWithBanner(Graphics2D g2d, Project project, BufferedImage banner, RoundRectangle2D cardShape, int x, int y, int w) {
        ...
        ProjectClassification type = project.getClassification() != null ? project.getClassification() : ProjectClassification.PLUGIN;
        int badgeX = x + w - 30;
        int badgeY = y + 30;
        drawCategoryBadge(g2d, type.name(), badgeX, badgeY);
    ...
    private void drawProjectContent(BufferedImage image, Graphics2D g2d, Project project, BufferedImage icon, int cardX, int cardY, int cardW, int cardH) {
        int headerH = cardW / 3;
        int iconSize = ICON_SIZE;

        int iconX = cardX + PROJECT_PADDING;
        int iconY = cardY + headerH - (iconSize / 2) - 20;
        drawIcon(image, g2d, project, icon, iconX, iconY, iconSize);
        ...
        String title = truncateText(g2d, project.getTitle(), cardW - (PROJECT_PADDING * 2));
        g2d.drawString(title, textX, titleY);

        int authorY = titleY + 60;
        g2d.setFont(new Font("SansSerif", Font.PLAIN, 32));
        g2d.setColor(TEXT_SECONDARY);
        g2d.drawString("by " + project.getAuthor(), textX, authorY);
        ...
        drawStatsBlock(g2d, statY, textX, project);
    }

    private void drawIcon(BufferedImage mainImage, Graphics2D g2d, Project project, BufferedImage img, int x, int y, int size) {
        ...
        try {
            if (img != null) {
                ...
                String l = project.getTitle().substring(0, 1).toUpperCase();
        ...
        g2d.setColor(ICON_BORDER);
        g2d.setStroke(new BasicStroke(10f));
        g2d.drawRoundRect(x, y, size, size, 40, 40);
    }
    ...
    private void drawCategoryBadge(Graphics2D g2d, String text, int rightX, int topY) {
        ...
        iconPath.moveTo(iconX + 7, iconY - 6);
        iconPath.lineTo(iconX + 1, iconY);
        iconPath.lineTo(iconX + 7, iconY + 6);

        iconPath.moveTo(iconX + 14, iconY - 6);
        iconPath.lineTo(iconX + 20, iconY);
        iconPath.lineTo(iconX + 14, iconY + 6);
        g2d.draw(iconPath);
    ...
    private void drawStatsBlock(Graphics2D g2d, int y, int leftX, Project project) {
        g2d.setFont(new Font("SansSerif", Font.BOLD, STAT_VALUE_FONT_SIZE));
        g2d.setColor(TEXT_SECONDARY);

        String downloadVal = formatNumber(project.getDownloadCount());
        String heartVal = formatNumber(project.getFavoriteCount());
