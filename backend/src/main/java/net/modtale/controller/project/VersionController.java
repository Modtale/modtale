...
public class VersionController {
    ...
    public ResponseEntity<Resource> downloadWithToken(@PathVariable String token, HttpServletRequest request) {
        try {
            ...
            Project project = projectService.getProjectById(dt.getProjectId());
            if (project == null) return ResponseEntity.notFound().build();
            ...
            String filename = targetVersion.getFileUrl().contains("/") ? targetVersion.getFileUrl().substring(targetVersion.getFileUrl().lastIndexOf('/') + 1) : targetVersion.getFileUrl();
            if (filename.length() > 37 && filename.charAt(36) == '-') filename = filename.substring(37);
            return ResponseEntity.ok().header(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=\"" + filename + "\"").contentType(MediaType.APPLICATION_OCTET_STREAM).body(new ByteArrayResource(data));
