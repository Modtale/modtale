...
class ReportControllerTest {
    ...
    void setUp() {
        ...
        controller = new ReportController(reportService, accountService);
    }
    ...
    void submitReportReturnsTheCreatedReportId() {
        ...
        when(accountService.requireCurrentUser("submitting a report")).thenReturn(reporter);
        when(reportService.createReport("project-1", Report.TargetType.PROJECT, "Spam", "Repeated reposts", reporter))
                .thenReturn(report);

        var response = controller.submitReport(request);
