...
class ReviewRepairAccessTest {
    ...
    @Test void neitherReadNorRescanAloneAuthorizesAnyRepairOperation() {
        ...
        for(var permissions:List.of(Set.of(AdminPermission.PROJECT_REVIEW_READ),Set.of(AdminPermission.PROJECT_VERSION_RESCAN),Set.of(AdminPermission.PROJECT_REVIEW_DECIDE))) {
            actor.setAdminPermissions(permissions);assertThrows(SecurityException.class,()->access.closeExpired(prepared));assertThrows(SecurityException.class,()->access.operations(null,25));assertThrows(SecurityException.class,()->access.recover(prepared.id()));assertThrows(SecurityException.class,()->access.capability());
