...
class ModerationQueueCursorTest {
    ...
    @Test void filteredCursorPreservesScope() {
        for(var filter:ModerationQueuePageReader.Filter.values()) {
            var value=new ModerationQueuePageReader.Cursor("project",3,filter);
            assertEquals(value,ModerationQueueCursor.decode(ModerationQueueCursor.encode(value)));
        }
        assertThrows(IllegalArgumentException.class,()->ModerationQueueCursor.decode("2.ALL.s.0.YQ"));
    }
