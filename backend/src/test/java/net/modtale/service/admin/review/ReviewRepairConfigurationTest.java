...
class ReviewRepairConfigurationTest {
    ...
    @Test void featureIsAbsentByDefault() {
        new ApplicationContextRunner().withUserConfiguration(ReviewRepairConfiguration.class).run(context->assertFalse(context.containsBean("reviewRepairPreparation")));
    }
    ...
    @Test void configuredFeatureConstructsBoundedPreparationWithoutStartingWork() {
        ...
        new ApplicationContextRunner().withUserConfiguration(ReviewRepairConfiguration.class).withBean(MongoTemplate.class,()->mongo)
                ...
                .run(context->{assertNull(context.getStartupFailure());assertNotNull(context.getBean(ReviewRepairPreparation.class));assertNotNull(context.getBean(ReviewSnapshotArchive.class));});
    }
