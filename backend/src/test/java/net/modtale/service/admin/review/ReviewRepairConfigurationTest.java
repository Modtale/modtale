...
class ReviewRepairConfigurationTest {
    ...
    @Test void remoteAdmissionUsesTheExistingRuntimeWithoutDispatching() {
        ...
        new ApplicationContextRunner().withUserConfiguration(ReviewRepairConfiguration.class).withBean(MongoTemplate.class,()->mongo)
                ...
                .run(context->{assertNull(context.getStartupFailure());assertNotNull(context.getBean(ProjectMutationAdmissionPreparation.class));assertNotNull(context.getBean(ProjectMutationActivator.class));assertNotNull(context.getBean(ProjectMutationAutomaticAdmission.class));assertNotNull(context.getBean(ProjectMutationPriorWorkReader.class));verifyNoInteractions(remote);});
    }
