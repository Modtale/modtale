...
export async function invalidateFrontend(env, { request = fetch, wait = ms => new Promise(r => setTimeout(r, ms)) } = {}) {
  ...
  const publicPages = [new URL('/', publicUrl).href, new URL('/launcher', publicUrl).href];
  ...
  await purge({ tags: [`modtale-html-${host}`] });
  // Older HTML was cached before it carried a cache tag.
  await purge({ files: publicPages });
  // Use ordinary public URLs: cache-busting queries would hide stale-cache bugs.
  for (const page of publicPages) await verify(page, 12);
