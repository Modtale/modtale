...
function Host({ enabled = true, subject = 'user', filter = 'ALL' }: { enabled?: boolean; subject?: string; filter?: 'ALL'|'SECURITY'|'OPERATIONS' }) { state = useModerationQueue(enabled, subject, filter); return <span>{state.page.items.map(row => row.title).join(',')}</span>; }
...
it('replaces pages rather than accumulating an unbounded list', async () => {
    ...
    expect(container.textContent).toBe('second');expect(getModerationQueuePage).toHaveBeenLastCalledWith('cursor',expect.any(AbortSignal),'ALL');
});
...
it('retains the page on failed continuation and retries the failed cursor', async () => {
    ...
    await act(async () => { await state.retry(); });expect(getModerationQueuePage).toHaveBeenLastCalledWith('cursor',expect.any(AbortSignal),'ALL');expect(container.textContent).toBe('second');
});
...
it('filter changes clear the previous page and reject its late response', async () => {
    const old=deferred();vi.mocked(getModerationQueuePage).mockReturnValueOnce(old.promise).mockResolvedValueOnce({...page('security'),filter:'SECURITY'});
    await act(async () => root.render(<Host />));await act(async () => root.render(<Host filter="SECURITY" />));
    expect(getModerationQueuePage).toHaveBeenLastCalledWith(null,expect.any(AbortSignal),'SECURITY');
    await act(async () => old.resolve(page('old-all')));expect(container.textContent).toBe('security');
});
