...
const SortDropdown = ({ value, onChange, onOpen, isMobile }: { value: string, onChange: (val: any) => void, onOpen: () => void, isMobile: boolean }) => {
    ...
    return (
        <div className="relative w-full md:w-auto" ref={containerRef}>
            ...
            {isOpen && (
                <div className="absolute right-0 md:right-auto md:left-0 top-full mt-2 w-48 bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/10 rounded-2xl shadow-xl py-2 z-[70] animate-in fade-in zoom-in-95 duration-200">
                        ...
                        {totalPages > 1 && (
                            <div className="mt-12 flex flex-col md:flex-row justify-center items-center gap-4 pb-12 animate-in fade-in slide-in-from-bottom-4 duration-700 delay-300">
                                <div className="flex items-center gap-2">
                                    {page === 0 ? (
                                        <button disabled aria-label="Previous page" className="w-11 h-11 flex items-center justify-center rounded-xl border border-slate-200 dark:border-white/10 text-slate-600 dark:text-slate-400 opacity-50 cursor-not-allowed transition-colors"><ChevronLeft className="w-5 h-5" /></button>
                                    ) : (
                                        <Link to={createPageUrl(page - 1)} onClick={handleScrollTop} aria-label="Previous page" className="w-11 h-11 flex items-center justify-center rounded-xl border border-slate-200 dark:border-white/10 text-slate-600 dark:text-slate-300 bg-white/40 dark:bg-white/5 hover:bg-white/60 dark:hover:bg-white/[0.02] transition-colors shadow-sm"><ChevronLeft className="w-5 h-5" /></Link>
                                    )}

                                    <div className="hidden sm:flex gap-2">
                                        {getPageNumbers().map((p, idx) => (
                                            typeof p === 'number' ? (
                                                <Link key={p} to={createPageUrl(p - 1)} onClick={handleScrollTop} aria-label={`Page ${p}`} className={`w-11 h-11 flex items-center justify-center rounded-xl text-sm font-bold border transition-colors ${page === p - 1 ? 'bg-modtale-accent text-white border-transparent shadow-md' : 'bg-white/40 dark:bg-white/5 text-slate-600 dark:text-slate-300 border-slate-200 dark:border-white/10 hover:bg-white/60 dark:hover:bg-white/[0.02] shadow-sm'}`}>{p}</Link>
                                            ) : (
                                                <span key={`dots-${idx}`} className="w-11 h-11 flex items-center justify-center text-slate-400">...</span>
                                            )
                                        ))}
                                    </div>

                                    {page === totalPages - 1 ? (
                                        <button disabled aria-label="Next page" className="w-11 h-11 flex items-center justify-center rounded-xl border border-slate-200 dark:border-white/10 text-slate-600 dark:text-slate-400 opacity-50 cursor-not-allowed transition-colors"><ChevronRight className="w-5 h-5" /></button>
                                    ) : (
                                        <Link to={createPageUrl(page + 1)} onClick={handleScrollTop} aria-label="Next page" className="w-11 h-11 flex items-center justify-center rounded-xl border border-slate-200 dark:border-white/10 text-slate-600 dark:text-slate-300 bg-white/40 dark:bg-white/5 hover:bg-white/60 dark:hover:bg-white/[0.02] transition-colors shadow-sm"><ChevronRight className="w-5 h-5" /></Link>
                                    )}
                                </div>
                                <div className="hidden md:block w-px h-6 bg-slate-200 dark:bg-white/10"></div>
                                <form onSubmit={handleJump} className="flex items-center gap-2">
                                    <span className="text-xs font-bold text-slate-600 dark:text-slate-400 uppercase drop-shadow-sm">Go to</span>
                                    <input type="number" min={1} max={totalPages} value={jumpPage} aria-label="Jump to page" onChange={(e) => setJumpPage(e.target.value)} className="w-14 h-11 rounded-xl border border-slate-200 dark:border-white/10 bg-white/40 dark:bg-white/5 shadow-sm px-1 text-sm font-bold text-center text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-modtale-accent transition-all [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none" placeholder="#" />
                                    <button type="submit" disabled={!jumpPage} aria-label="Go to page" className="w-11 h-11 flex items-center justify-center rounded-xl bg-white/40 dark:bg-white/5 border border-slate-200 dark:border-white/10 hover:border-modtale-accent/50 shadow-sm hover:bg-modtale-accent hover:text-white dark:hover:bg-modtale-accent text-slate-500 dark:text-slate-400 transition-colors disabled:opacity-50 flex items-center justify-center"><CornerDownLeft className="w-4 h-4" /></button>
                                </form>
                            </div>
                        )}
                    </div>
