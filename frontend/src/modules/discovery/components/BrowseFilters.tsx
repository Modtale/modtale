...
export const BrowseFilters: React.FC<BrowseFiltersProps> = React.memo(({
    ...
    const filterMenuBody = (
        <div className="space-y-5">
            <details className="group/tags space-y-2">
                <summary className="list-none cursor-pointer flex items-center justify-between rounded-xl border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-slate-950/50 px-3 py-2 text-sm font-bold text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-white/5 [&::-webkit-details-marker]:hidden">
                    <span>Tags{selectedTags.length > 0 ? ` (${selectedTags.length})` : ''}</span>
                    <ChevronDown className="w-4 h-4 text-slate-400 transition-transform group-open/tags:rotate-180" />
                </summary>
