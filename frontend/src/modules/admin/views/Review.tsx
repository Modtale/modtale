                        ...
                        {currentStep === 2 && (
                            <div className="max-w-3xl mx-auto space-y-8 animate-in slide-in-from-right-4 duration-300">
                                ...
                                {scanResult?.status === 'FAILED' && <div role="status" className="rounded-2xl border border-amber-300 p-5 text-sm text-amber-800 dark:text-amber-200">
                                    ...
                                    <p>{['REMOTE_BINDING_MISSING', 'REMOTE_BINDING_MISMATCH'].includes(scanResult.scanState || '') ? 'The stored review job is missing or no longer matches this version. Repair review state before retrying.' : scanResult.scanState === 'REMOTE_UNSUPPORTED_CONTEXT' ? 'The current dependencies, runtime metadata or supplemental content cannot be fully reviewed. Resolve the review context before requesting another scan.' : scanResult.scanState === 'REMOTE_EXPIRED' ? 'The review expired.' : scanResult.scanState === 'REMOTE_CANCELLED' ? 'The review was cancelled.' : scanResult.scanState === 'REMOTE_HELD' ? 'The review was held.' : 'The review is unavailable.'}</p>
                                </div>}
                                                    ...
                                                    >
                                                        ...
                                                        <div className="min-w-0">
                                                            <div className="text-sm font-bold text-slate-700 dark:text-slate-300 truncate group-hover:text-modtale-accent">
                                                                {meta?.title || d.projectId}
                                                            </div>
                                                            <div className="text-[10px] text-slate-400 font-mono">{d.projectId}</div>
                                                        </div>
