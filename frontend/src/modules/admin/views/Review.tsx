                        ...
                        {currentStep === 2 && (
                            <div className="max-w-3xl mx-auto space-y-8 animate-in slide-in-from-right-4 duration-300">
                                ...
                                {scanResult?.status === 'FAILED' && <div role="status" className="rounded-2xl border border-amber-300 p-5 text-sm text-amber-800 dark:text-amber-200">
                                    ...
                                    <p>{scanResult.scanState === 'REMOTE_UNSUPPORTED_CONTEXT' ? 'The current dependencies, runtime metadata or supplemental content cannot be fully reviewed. Resolve the review context before requesting another scan.' : scanResult.scanState === 'REMOTE_EXPIRED' ? 'The review expired.' : scanResult.scanState === 'REMOTE_CANCELLED' ? 'The review was cancelled.' : scanResult.scanState === 'REMOTE_HELD' ? 'The review was held.' : 'The review is unavailable.'}</p>
                                </div>}
