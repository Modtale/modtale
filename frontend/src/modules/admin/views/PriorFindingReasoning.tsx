...
function PriorFindingReasoningSnapshot({ projectId, versionId, token, issues, sources, issueIndex, sourceVersionId, autoLoad = false }: Props) {
    ...
    const canLoad = !!token && sources.some(value => value.id === source) && Number.isInteger(issue) && issue >= 0 && issue < issues.length;
    ...
    useEffect(() => {
        generation.current++; inFlight.current = false; setResult(null); setBusy(false); setError('');
        return () => { generation.current++; };
    }, [projectId, versionId, token, source, issue]);
    async function load() {
        if (inFlight.current || !canLoad) return;
    ...
    useEffect(() => {
        if (autoLoad && canLoad) void load();
    }, [autoLoad, projectId, versionId, token, source, issue, canLoad]);
    ...
    return <section aria-label="Earlier finding reasoning" className="mt-4 rounded-lg border border-slate-300 dark:border-slate-700 p-4 space-y-3">
        ...
        <button type="button" disabled={busy || !canLoad} onClick={() => void load()} className="text-sm font-bold">{busy ? 'Loading earlier reasoning…' : 'Find earlier reasoning'}</button>
