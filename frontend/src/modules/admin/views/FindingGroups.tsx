...
export function FindingGroups({ issues, selected, onSelect }: Props) {
    ...
    return <section aria-label="Finding groups" className="rounded-lg border border-slate-300 dark:border-slate-700 p-3 text-sm dark:text-slate-200">
        <details>
            <summary className="cursor-pointer font-bold">Finding groups ({groups.length} types)</summary>
            ...
            {pages > 1 && <nav aria-label="Finding group pages" className="mt-2 flex gap-3">
                <button type="button" disabled={visiblePage === 0} onClick={() => setPage(visiblePage - 1)}>Previous groups</button>
                <span>Group page {visiblePage + 1} of {pages}</span>
                <button type="button" disabled={visiblePage + 1 >= pages} onClick={() => setPage(visiblePage + 1)}>Next groups</button>
            </nav>}
        </details>
        {selected !== null && <p className="mt-2 break-all">Selected type: {selected}. <button type="button" onClick={() => onSelect(null)} className="font-bold text-indigo-600 dark:text-indigo-300">Show all types</button></p>}
    </section>;
}
