                    ...
                    <div key={report.id} className="bg-white/40 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-3xl backdrop-blur-md p-6 flex flex-col md:flex-row gap-6 relative overflow-hidden group">
                        ...
                        <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-3 mb-2 flex-wrap">
                                <span className={`px-2 py-0.5 rounded text-[10px] font-black uppercase tracking-wider border ${report.status === 'OPEN' ? 'bg-red-500/10 text-red-500 border-red-500/20' : report.status === 'RESOLVED' ? 'bg-green-500/10 text-green-500 border-green-500/20' : 'bg-slate-500/10 text-slate-500 border-slate-500/20'}`}>
                            ...
                            <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-1">{report.targetSummary || 'Unknown Target'}</h3>
                        ...
                        <div className="flex flex-col gap-2 justify-center min-w-[180px]">
                            {report.targetType !== 'COMMENT' && (
                                <a href={getTargetLink(report)} target="_blank" rel="noreferrer" className="w-full py-2 bg-slate-100 dark:bg-white/5 text-slate-600 dark:text-slate-300 rounded-lg text-xs font-bold flex items-center justify-center gap-2 hover:bg-slate-200 dark:hover:bg-white/10 transition-colors">
