...
export const VersionFields: React.FC<VersionFieldsProps> = ({ data, onChange, isModpack, projectType, existingVersions = [], previousDependencies, disabled, hideFilePicker = false, currentProjectId }) => {
    ...
    return (
        <div className={`space-y-8 ${disabled ? 'opacity-60 pointer-events-none' : ''}`}>
            {!hideFilePicker && (
                <details open={isModpack ? undefined : true} className="group/upload">
                    <summary className={isModpack ? `cursor-pointer text-sm font-bold ${theme.colors.textSecondary} mb-3` : 'hidden'}>Advanced: upload a configs &amp; overrides ZIP</summary>
                    <Label required={!isModpack}>{isModpack ? 'Configs & Overrides' : 'Project File'} <span className={`${theme.colors.textSecondary} font-normal normal-case ml-1`}>{allowsAutoSwitch ? '(.jar or .zip)' : '(.zip)'}</span></Label>
                    {isModpack && <p className={`mb-2 text-xs ${theme.colors.textSecondary}`}>Optional ZIP mirroring your Hytale UserData folders beneath overrides/. For world-specific plugin configs, use overrides/Saves/&lt;world&gt;/mods/&lt;plugin-folder&gt;/config.json. Preserve the folder names and capitalization generated by the mod.</p>}
                    <div
                        {...getRootProps()}
            ...
            {isModpack && (
                <div className="mt-4">
                    ...
                    <p className={`text-xs ${theme.colors.textSecondary} mb-2`}>Select the mods to include, then optionally attach their config files and choose where they install.</p>
                    <DependencySelector
                        selectedDeps={data.dependencies || []}
                        onChange={(deps) => onChange({ ...data, dependencies: deps as ProjectDependency[], modConfigs: (data.modConfigs || []).filter(config => deps.some(dep => dep.projectId === config.projectId)) })}
                        targetGameVersion={data.gameVersions?.[0]}
                        label="Add Projects"
                        isModpack={true}
                        renderDependencyDetails={hideFilePicker ? undefined : dependency => <ModConfigFields
                            ...
                            configs={(data.modConfigs || []).filter(config => config.projectId === dependency.projectId)}
                            onChange={configs => onChange({ ...data, modConfigs: [...(data.modConfigs || []).filter(config => config.projectId !== dependency.projectId), ...configs] })}
                            disabled={disabled}
                        />}
                        disabled={disabled}
                    />
