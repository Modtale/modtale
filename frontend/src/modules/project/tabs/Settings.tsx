        ...
        <div className="space-y-6">
            <div className={`${theme.colors.bgSurface} p-6 rounded-2xl border ${theme.colors.border}`}>
                {(projectData?.status === 'PUBLISHED' || projectData?.status === 'UNLISTED') && (
                    <div className={`mb-6 pb-6 border-b ${theme.colors.borderFaint}`}>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            ...
                            <button type="button" onClick={handleUnlist} disabled={isLoading || projectData.status === 'UNLISTED' || !hasProjectPermission('PROJECT_STATUS_UNLIST')} className={`p-4 rounded-xl border flex flex-col items-center gap-3 transition-all ${projectData.status === 'UNLISTED' ? 'bg-orange-500/10 border-orange-500 text-orange-500' : `${theme.colors.bgBase} ${theme.colors.border} hover:border-orange-500 hover:text-orange-500 disabled:opacity-50`}`}>
                            ...
                            <button type="button" onClick={handleArchive} disabled={isLoading || !hasProjectPermission('PROJECT_STATUS_ARCHIVE')} className={`p-4 rounded-xl border flex flex-col items-center gap-3 transition-all ${theme.colors.bgBase} ${theme.colors.border} hover:border-slate-500 hover:text-slate-500 disabled:opacity-50`}>
                ...
                <div className="flex items-center justify-between mb-4">
                    ...
                    <button type="button" disabled={readOnly || !hasProjectPermission('PROJECT_EDIT_METADATA')} onClick={() => { markDirty(); setProjectData(prev => prev ? {...prev, allowModpacks: !prev.allowModpacks} : null); }} className={`transition-colors ${readOnly || !hasProjectPermission('PROJECT_EDIT_METADATA') ? 'opacity-50' : projectData?.allowModpacks ? 'text-green-500' : theme.colors.textSecondary}`}>{projectData?.allowModpacks ? <ToggleRight className="w-8 h-8" /> : <ToggleLeft className="w-8 h-8" />}</button>
                </div>
                ...
                <div className="flex items-center justify-between mb-4">
                    ...
                    <button type="button" disabled={readOnly || !hasProjectPermission('PROJECT_EDIT_METADATA')} onClick={() => { markDirty(); setProjectData(prev => prev ? {...prev, hmWikiEnabled: !prev.hmWikiEnabled} : null); }} className={`transition-colors ${readOnly || !hasProjectPermission('PROJECT_EDIT_METADATA') ? 'opacity-50' : projectData?.hmWikiEnabled ? 'text-green-500' : theme.colors.textSecondary}`}>{projectData?.hmWikiEnabled ? <ToggleRight className="w-8 h-8" /> : <ToggleLeft className="w-8 h-8" />}</button>
                </div>
                ...
                {projectData?.hmWikiEnabled && (
                    <div className={`mb-4 p-4 ${theme.colors.bgSurfaceAlt} rounded-xl border ${theme.colors.border} animate-in slide-in-from-top-2`}>
                        ...
                        <input value={projectData.hmWikiSlug || ''} onChange={e => { markDirty(); const newSlug = e.target.value; setProjectData(prev => prev ? {...prev, hmWikiSlug: newSlug} : null); setMetaData(prev => { const currentWiki = prev.links.WIKI || ''; if (!currentWiki || /^https?:\/\/wiki\.hytalemodding\.dev\/mods?\//i.test(currentWiki)) { return { ...prev, links: { ...prev.links, WIKI: newSlug ? `https://wiki.hytalemodding.dev/mod/${newSlug}` : '' } }; } return prev; }); }} disabled={readOnly || !hasProjectPermission('PROJECT_EDIT_METADATA')} placeholder="e.g., my-awesome-mod" className={`w-full ${theme.colors.bgBase} border ${theme.colors.border} rounded-lg px-3 py-2 text-sm font-mono focus:border-modtale-accent focus:ring-1 focus:ring-modtale-accent outline-none transition-all`} />
                    </div>
                )}
            </div>
            {!readOnly && hasProjectPermission('PROJECT_DELETE') && <button type="button" onClick={handleDelete} className="w-full bg-red-500/10 border border-red-500/20 text-red-500 hover:bg-red-500 hover:text-white p-4 rounded-xl font-bold flex justify-center gap-2 transition-all"><Trash2 className="w-4 h-4"/> Delete Project</button>}
        </div>
    );
};