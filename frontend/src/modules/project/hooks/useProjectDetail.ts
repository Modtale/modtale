...
export const useProjectDetail = (rawId: string | undefined, initialData: Project | null, currentUser: User | null) => {
    ...
    useEffect(() => {
        ...
        const fetchMeta = async () => {
            const missing = latestDependencies.filter(d => !depMeta[d.projectId] && !fetchedDepMeta.current.has(d.projectId));
            ...
            await Promise.all(missing.map(async (d) => {
                try {
                    ...
                    newMeta[d.projectId] = { icon: data.icon, title: data.title };
                } catch (e) {
