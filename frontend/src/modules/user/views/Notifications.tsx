...
export function Notifications({ user }: NotificationsProps) {
    ...
    const handleSave = async () => {
        setSaving(true);
        try {
            await userClient.updateNotifications({ projectUpdates, creatorUploads, newComments, newFollowers, dependencyUpdates });
            setSaved(true);
                ...
                <div className="relative flex bg-white/60 dark:bg-black/20 border border-slate-200 dark:border-white/10 p-1 rounded-xl shadow-inner shrink-0 w-fit">
                    <div
                        className={`absolute top-1 bottom-1 w-16 rounded-lg transition-all duration-300 ease-out shadow-sm ${
                            isOn ? 'bg-modtale-accent shadow-modtale-accent/30 border-transparent' : 'bg-white dark:bg-slate-700 border border-slate-200 dark:border-white/10'
                        }`}
                        style={{ transform: `translateX(${isOn ? 100 : 0}%)` }}
                    />
                    <button onClick={() => onChange('OFF')} className={`relative z-10 w-16 py-2 text-xs font-bold transition-colors duration-300 ${!isOn ? 'text-slate-900 dark:text-white' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-300'}`}>Off</button>
                    <button onClick={() => onChange('ON')} className={`relative z-10 w-16 py-2 text-xs font-bold transition-colors duration-300 ${isOn ? 'text-white' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-300'}`}>On</button>
                </div>
            </div>
        );
    };
        ...
        <div className="space-y-8">
            <div className="space-y-6">
                ...
                <div className="flex justify-end">
                    <button onClick={handleSave} disabled={saving} className="bg-modtale-accent text-white px-8 py-3 rounded-xl font-bold hover:bg-modtale-accentHover transition-all shadow-lg shadow-modtale-accent/20 flex items-center gap-2 disabled:opacity-70 active:scale-95">
