...
const order=['void-scythe','hexcode','autosort','capybara-plushie','waypoints','levelingcore','ev0s-smokable-herbs','more-weapons-more-armor','infuse','better-repair-kits','implement-weapons','angler-s-almanac','torches','more-armor','dub-s-cannabinol'];
for(const kind of ['modpacks','launcher']){
 ...
 if(kind==='modpacks'){
  ...
  for(let col=0;col<4;col++)for(let row=0;row<4;row++){
   ...
   const name=order[n%order.length];body+=hex(`m${n++}`,x,y,97,icons[name]);
  }
  ...
  for(let i=0;i<10;i++)body+=hex(`l${i}`,680+(i%5)*142,70+Math.floor(i/5)*480,82,icons[order[i]],.75);
